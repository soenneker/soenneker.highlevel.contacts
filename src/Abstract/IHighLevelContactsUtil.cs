...
public interface IHighLevelContactsUtil
{
    ...
    ValueTask<UpsertContactsSuccessfulResponseDto?> Upsert(UpsertContactDto contact, CancellationToken cancellationToken = default);
    ...
    ValueTask<SearchPostResponse?> Search(SearchBodyV2DTO searchBody, CancellationToken cancellationToken = default);
    ...
    ValueTask<ContactsByIdSuccessfulResponseDto?> GetById(string contactId, CancellationToken cancellationToken = default);
    ...
    /// <param name="cancellationToken"></param>
    /// <returns>A task that represents the asynchronous operation. The task result contains the contact if found, or null if not found.</returns>
    ValueTask<GetContectByIdSchema?> GetByEmail(string email, string locationId, CancellationToken cancellationToken = default);
    ...
    ValueTask<UpdateContactsSuccessfulResponseDto?> Update(string contactId, UpdateContactDto updateDto, CancellationToken cancellationToken = default);
    ...
    ValueTask<DeleteContactsSuccessfulResponseDto?> Delete(string contactId, CancellationToken cancellationToken = default);
}
